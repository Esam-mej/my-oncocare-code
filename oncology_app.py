...
MALIGNANCIES = [
    ...
    "BRAIN T", "RHABDO", "RETINO", "HEPATO", "GERM CELL", "OTHER"
]
...
MALIGNANCY_FIELDS = {
    ...
    "OTHER": [
        "MALIGNANCY_NAME",  # This is the custom field for user input
        "RISK_GROUP", "STAGE", "HISTOPATHOLOGY", "CYTOGENETIC", "SURGERY", 
        "SR_TYPE", "SR_DATE", "TREATMENT_STAGE", "CYCLE", 
        "RADIOTHERAPY", "BMT", "STATE", "THERAPY_SIDE_EFFECTS", 
        "NOTES"
...
class GoogleDriveManager:
    ...
    def upload_file(self, file_path, file_name, folder_id=None):
        ...
        try:
            ...
            existing_files = self.service.files().list(q=query, fields="files(id, parents, modifiedTime)").execute().get('files', [])
            ...
            if existing_files:
                ...
                if local_mtime > drive_mtime:
                    # If folder_id is not in parents, move the file
                    current_parents = existing_files[0].get('parents', [])
                    add_parents = []
                    remove_parents = []
                    if folder_id and folder_id not in current_parents:
                        add_parents.append(folder_id)
                    ...
                    if add_parents:
                        update_kwargs['addParents'] = ','.join(add_parents)
                    if remove_parents:
                        update_kwargs['removeParents'] = ','.join(remove_parents)

                    file = self.service.files().update(**update_kwargs).execute()
...
class FirebaseManager:
    ...
    def sync_json_file(self, filename, local_path):
        """Sync a JSON file between local and Firebase (latest version wins)."""
        if not self.initialized:
            return False, "Firebase not initialized"

        try:
            # Get remote file and timestamp
            doc_ref = self.db.collection('shared_files').document(filename)
            remote_doc = doc_ref.get()
            remote_data = remote_doc.to_dict() if remote_doc.exists else None
            remote_time = remote_data.get('LAST_MODIFIED') if remote_data else None

            # Get local file and timestamp
            ...
            # Compare timestamps (use ISO format for remote, epoch for local)
            if remote_time:
                remote_epoch = datetime.fromisoformat(remote_time).timestamp()
            else:
                remote_epoch = 0

            # Decide which version to keep
            if local_time > remote_epoch:
                # Local is newer: upload to Firebase
                doc_ref.set({
                    'content': local_content,
                    'LAST_MODIFIED': datetime.now().isoformat()
                })
                return True, "Uploaded local file to Firebase"
            elif remote_epoch > local_time:
                # Remote is newer: download from Firebase
                if remote_data and 'content' in remote_data:
                    with open(local_path, 'w', encoding='utf-8') as f:
                        f.write(remote_data['content'])
                    os.utime(local_path, (remote_epoch, remote_epoch))
                return True, "Downloaded latest file from Firebase"
            else:
...
class OncologyApp:
    ...
    def init_cloud_services(self):
        ...
        try:
            ...
            if self.firebase.initialized:
                ...
                self.sync_shared_files()  # <-- ADD THIS LINE HERE
            else:
    ...
    def manage_drug_json(self):
        ...
        def save_drug_details():
            ...
            try:
                ...
                show_drug_details()
                # Reload in main app and sync to Firebase
                if file_var.get() == "chemo_drugs.json":
                    self.chemo_drugs = load_chemo_drugs()
                    if self.firebase and self.firebase.initialized:
                        self.firebase.sync_json_file("chemo_drugs.json", "chemo_drugs.json")
                elif file_var.get() == "antibiotics.json":
                    self.antibiotics_data = load_antibiotics_data()
                    if self.firebase and self.firebase.initialized:
                        self.firebase.sync_json_file("antibiotics.json", "antibiotics.json")
    ...
    def main_menu(self):
        ...
        if self.users[self.current_user]["role"] in ["admin"]:
         ...
         # OPD Appointments button
        ttk.Button(patient_frame, text="📅 OPD Appointments", command=self.manage_appointments,
                  style='Accent.TButton').pack(fill=tk.X, pady=2)
    ...
    def save_chemo_drugs(self, drugs):
        with open('chemo_drugs.json', 'w', encoding='utf-8') as f:
            json.dump(drugs, f, indent=2, ensure_ascii=False)
        if self.firebase and self.firebase.initialized:
            self.firebase.sync_json_file("chemo_drugs.json", "chemo_drugs.json")

    def save_antibiotics_data(self, antibiotics):
        with open('antibiotics.json', 'w', encoding='utf-8') as f:
            json.dump(antibiotics, f, indent=2, ensure_ascii=False)
        if self.firebase and self.firebase.initialized:
            self.firebase.sync_json_file("antibiotics.json", "antibiotics.json")
    ...
    def sync_shared_files(self):
        """Sync all shared JSON files with Firebase (latest version wins)."""
        shared_files = [
            ("antibiotics.json", "antibiotics.json"),
            ("appointments.json", "appointments.json"),
            ("chemo_drugs.json", "chemo_drugs.json")
        ]
        for remote_name, local_path in shared_files:
            if self.firebase and self.firebase.initialized:
                self.firebase.sync_json_file(remote_name, local_path)

    def manage_appointments(self):
        """Modern OPD appointment manager."""
        self.clear_frame()
        main_frame = ttk.Frame(self.root, style='TFrame')
        main_frame.pack(fill=tk.BOTH, expand=True, padx=40, pady=40)

        # Title
        ttk.Label(main_frame, text="Outpatient Department (OPD) Appointments",
        ...
        self.opd_date_var = tk.StringVar(value=datetime.now().strftime("%d/%m/%Y"))
        date_entry = ttk.Entry(date_frame, textvariable=self.opd_date_var, width=12)
        date_entry.pack(side=tk.LEFT, padx=5)
        ttk.Button(date_frame, text="📅", command=lambda: self.show_calendar(date_entry)).pack(side=tk.LEFT)

        ttk.Button(date_frame, text="Show Appointments", command=self.show_appointments_for_date).pack(side=tk.LEFT, padx=10)
        ttk.Button(date_frame, text="Add Appointment", command=self.add_appointment_window).pack(side=tk.LEFT, padx=10)
        ttk.Button(date_frame, text="Back", command=self.main_menu).pack(side=tk.LEFT, padx=10)

        # Table for appointments
        self.opd_table_frame = ttk.Frame(main_frame)
        self.opd_table_frame.pack(fill=tk.BOTH, expand=True, pady=20)
        self.show_appointments_for_date()
    ...
    def save_appointments(self, appointments):
        ...
        if self.firebase and self.firebase.initialized:
            self.firebase.sync_json_file("appointments.json", "appointments.json")

    def show_appointments_for_date(self):
        """Display appointments for the selected date with Edit and Delete buttons."""
        for widget in self.opd_table_frame.winfo_children():
            widget.destroy()
        date = self.opd_date_var.get()
        appointments = [a for a in self.load_appointments() if a["DATE"] == date]
        columns = ["TIME", "FILE_NUMBER", "NAME", "REASON", "REQUESTS"]
        col_widths = {"TIME": 10, "FILE_NUMBER": 12, "NAME": 18, "REASON": 18, "REQUESTS": 40}
        header = ttk.Frame(self.opd_table_frame)
        header.pack(fill=tk.X)
        for col in columns:
            ttk.Label(header, text=col, font=('Helvetica', 10, 'bold'), width=col_widths[col]).pack(side=tk.LEFT, padx=2)
        ttk.Label(header, text="Actions", font=('Helvetica', 10, 'bold'), width=16).pack(side=tk.LEFT, padx=2)
        for idx, appt in enumerate(sorted(appointments, key=lambda x: x.get("TIME", ""))):
            ...
            ttk.Button(btn_frame, text="Delete", width=7, command=lambda i=idx: self.delete_appointment(i)).pack(side=tk.LEFT, padx=1)

    def edit_appointment_window(self, appt_index):
        """Show window to edit an existing appointment."""
        date = self.opd_date_var.get()
        appointments = [a for a in self.load_appointments() if a["DATE"] == date]
        if appt_index < 0 or appt_index >= len(appointments):
            messagebox.showerror("Error", "Invalid appointment selected.")
            return
        appt = appointments[appt_index]
        win = tk.Toplevel(self.root)
        win.title("Edit OPD Appointment")
        win.geometry("400x550")
        frame = ttk.Frame(win, padding=20)
        frame.pack(fill=tk.BOTH, expand=True)

        ttk.Label(frame, text="File Number:").pack(anchor="w")
        file_var = tk.StringVar(value=appt.get("FILE_NUMBER", ""))
        file_entry = ttk.Entry(frame, textvariable=file_var)
        file_entry.pack(fill=tk.X, pady=3)

        ttk.Label(frame, text="Patient Name:").pack(anchor="w")
        name_var = tk.StringVar(value=appt.get("NAME", ""))
        ...
        date_var = tk.StringVar(value=appt.get("DATE", ""))
        date_entry = ttk.Entry(frame, textvariable=date_var)
        date_entry.pack(fill=tk.X, pady=3)
        ttk.Button(frame, text="📅", command=lambda: self.show_calendar(date_entry)).pack(anchor="e", pady=2)

        ttk.Label(frame, text="Time (HH:MM):").pack(anchor="w")
        time_var = tk.StringVar(value=appt.get("TIME", ""))
        time_entry = ttk.Entry(frame, textvariable=time_var)
        time_entry.pack(fill=tk.X, pady=3)

        ttk.Label(frame, text="Reason:").pack(anchor="w")
        reason_var = tk.StringVar(value=appt.get("REASON", ""))
        reason_entry = ttk.Entry(frame, textvariable=reason_var)
        reason_entry.pack(fill=tk.X, pady=3)

        ttk.Label(frame, text="Requests:").pack(anchor="w")
        requests_var = tk.StringVar(value=appt.get("REQUESTS", ""))
        requests_entry = ttk.Entry(frame, textvariable=requests_var)
        requests_entry.pack(fill=tk.X, pady=3)

        def save_edit():
            file_no = file_var.get().strip()
            name = name_var.get().strip()
            date_val = date_var.get().strip()
            time = time_var.get().strip()
            reason = reason_var.get().strip()
            requests = requests_var.get().strip()
            if not (file_no and name and date_val and time and reason):
                messagebox.showerror("Error", "All fields except requests are required.")
                return
            all_appts = self.load_appointments()
            # Find the correct appointment in all_appts by matching all fields (or by index in filtered list)
            filtered = [a for a in all_appts if a["DATE"] == self.opd_date_var.get()]
            if appt_index < 0 or appt_index >= len(filtered):
                messagebox.showerror("Error", "Could not find appointment to edit.")
                return
            idx_in_all = all_appts.index(filtered[appt_index])
            all_appts[idx_in_all] = {
                "FILE_NUMBER": file_no,
                "NAME": name,
                "DATE": date_val,
                "TIME": time,
            ...
            self.save_appointments(all_appts)
    ...
    def delete_appointment(self, appt_index):
        ...
        if appt_index < 0 or appt_index >= len(appointments):
            ...
            return
        appt = appointments[appt_index]
        if not messagebox.askyesno("Delete", f"Delete appointment for {appt.get('NAME', '')} at {appt.get('TIME', '')}?"):
            return
        all_appts = self.load_appointments()
        # Find the correct appointment in all_appts by matching all fields (or by index in filtered list)
        filtered = [a for a in all_appts if a["DATE"] == self.opd_date_var.get()]
        if appt_index < 0 or appt_index >= len(filtered):
            messagebox.showerror("Error", "Could not find appointment to delete.")
            return
        idx_in_all = all_appts.index(filtered[appt_index])
        del all_appts[idx_in_all]
        self.save_appointments(all_appts)
        messagebox.showinfo("Deleted", "Appointment deleted.")
        self.show_appointments_for_date()

    def add_appointment_window(self):
        """Show window to add a new appointment."""
        win = tk.Toplevel(self.root)
        win.title("Add OPD Appointment")
        win.geometry("400x550")
        frame = ttk.Frame(win, padding=20)
        frame.pack(fill=tk.BOTH, expand=True)
        ...
        ttk.Label(frame, text="Date (dd/mm/yyyy):").pack(anchor="w")
        date_var = tk.StringVar(value=self.opd_date_var.get())
        date_entry = ttk.Entry(frame, textvariable=date_var)
        date_entry.pack(fill=tk.X, pady=3)
        ttk.Button(frame, text="📅", command=lambda: self.show_calendar(date_entry)).pack(anchor="e", pady=2)
        ...
        def save_appt():
            ...
            reason = reason_var.get().strip()
            requests = requests_var.get().strip()
            if not (file_no and name and date and time and reason):
                messagebox.showerror("Error", "All fields except requests are required.")
                return
            ...
            self.save_appointments(appts)
            messagebox.showinfo("Success", "Appointment added.")
            win.destroy()
            self.show_appointments_for_date()

        ttk.Button(frame, text="Save Appointment", command=save_appt).pack(pady=10)
        ttk.Button(frame, text="Cancel", command=win.destroy).pack()
    ...
    def show_malignancy_fields(self, form_container):
        ...
        for field in MALIGNANCY_FIELDS.get(malignancy, []):
            ttk.Label(malignancy_frame, text=f"{field.replace('_', ' ').title()}:", anchor="w").grid(
                row=row, column=0, padx=5, pady=5, sticky="w")
            
            if field == "MALIGNANCY_NAME":
    ...
    def edit_patient(self, patient_data):
        ...
        if malignancy in MALIGNANCY_FIELDS:
            ...
            row = 0
            for field in MALIGNANCY_FIELDS[malignancy]:
                ttk.Label(specific_frame, text=f"{field.replace('_', ' ').title()}:", anchor="w").grid(row=row, column=0, padx=5, pady=5, sticky="w")
                current_value = patient_data.get(field, "")
                if field == "MALIGNANCY_NAME":
